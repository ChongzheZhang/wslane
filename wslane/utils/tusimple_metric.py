...
class LaneEval(object):
    ...
    def bench(pred, gt, y_samples, running_time, get_matches=False):
        '''
        Args:
            pred: list[list], only x values
            gt: list[list], only x values
            y_samples: list
            running_time: a number
        '''
        ...
        if get_matches:
            assert isinstance(my_matches, list)
            return my_matches, fp / len(pred) if len(pred) > 0 else 0., fn / max(min(len(gt), 4.), 1.)
        else:
