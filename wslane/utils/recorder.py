...
class Recorder(object):
    def __init__(self, cfg):
        ...
        self.no_comet = self.cfg.no_comet
        self.comet_experiment = comet_ml.Experiment(
            api_key="fniLSlgS68qWGpWfJGuuvJYS3",
            project_name="lane-segmentation",
            workspace="jzhou",
            disabled=self.no_comet,
        )
    ...
    def record_train(self):
        if hasattr(self, 'loss_stats'):
            # total_loss = 0
            for key in self.loss_stats.keys():
                # total_loss += self.loss_stats[key].avg
                self.comet_experiment.log_metric(key, self.loss_stats[key].avg, step=self.step, epoch=self.epoch)
            # self.comet_experiment.log_metric('total_loss', total_loss , step=self.step, epoch=self.epoch)
            self.comet_experiment.log_metric('lr', self.lr, step=self.step, epoch=self.epoch)

    def record_eval(self, eval_results):
        for key, value in eval_results.items():
            self.comet_experiment.log_metric(key, value, step=self.step, epoch=self.epoch)
