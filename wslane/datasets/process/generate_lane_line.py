...
from wslane.datasets.process.transforms import CLRTransforms
...
class GenerateLaneLine(object):
    ...
    def transform_annotation(self, anno, img_wh=None):
        ...
        if self.headname == 'CLRHead':
            lanes = np.ones((self.max_lanes, 2 + 1 + 1 + 2 + self.n_offsets),
                            dtype=np.float32) * -1e5  # 2 scores, 1 start_y, 1 start_x, 1 theta, 1 length, S+1 coordinates
            ...
            new_anno = {'label': lanes, 'old_anno': anno, 'number':lanes.sum(axis=0)[1]}
        return new_anno
